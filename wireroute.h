/**
 * Parallel VLSI Wire Routing via MPI this is the correct one
 * Name 1(mnguyen3), Name 2(akundu2)
 */
...
struct Wire {
  ...
  int num_bends;
  int bend_x[3], bend_y[3];
  validate_wire_t to_validate_format(void) const;
};

/** Serializable route update for MPI: wire index + new route (same layout as Wire endpoints/bends). */
struct RouteUpdate {
  int wire_id;
  int start_x, start_y, end_x, end_y;
  int num_bends;
